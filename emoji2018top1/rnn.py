...
ssp = StratifiedShuffleSplit(n_splits=1, test_size=opt.val_ratio)
...
#    m.summary()
    ...
    m.fit(x={'char_input': c_trn, 'word_input': w_trn},
          ...
          epochs=opt.epoch, callbacks=[prf_scores, early_stop], verbose=0)
    info("Results:({}) {}".format(np.max(prf_scores.val_f1), str(prf_scores)))
